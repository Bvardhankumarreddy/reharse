...
- main_prompt: ONE paragraph (≤ 800 chars) covering subject, composition,
  ...
  if they fit naturally. Plain English — no Midjourney parameters (no
  "--ar 16:9" etc.).
- face_position: "left" | "right" | "center" | "none". (Title text goes
  opposite the face.)
- text_overlay: ≤ 8 words. Drop articles. ALL CAPS or Title Case — pick
  what reads stronger.
- color_palette: name 3–4 hex colours (with #).
- mood: 2–3 words (e.g. "urgent and confident").
- style: e.g. "photoreal cinematic", "high-contrast editorial", "minimal
  flat-vector with bold accent".
- alternates: 3 ALTERNATIVE main_prompt strings — each a different angle
  on the same lesson, ≤ 600 chars.
...
export class ThumbnailAgent {
  ...
  async generateThumbnail(lessonId: string): Promise<ContentAsset> {
    ...
    const parsed = JSON.parse(result.text || '{}') as ThumbJson;
    ...
    this.logger.log(
      `Thumbnail v${asset.version} for lesson "${lesson.title}" — ` +
